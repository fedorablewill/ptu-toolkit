...
break;case "ice":a="rgb(130, 208, 208)";break;case "normal":a="rgb(158, 158, 109)";break;case "poison":a="rgb(149, 59, 149)";break;case "psychic":a="rgb(247, 64, 119)";break;case "rock":a="rgb(169, 147, 51)";break;case "steel":a="rgb(166, 166, 196)";break;case "water":a="rgb(82, 127, 238)"}return a};$.getScript("js/script.js");$.getScript("js/ptu-io.js");$.getScript("js/ptu-char.js");$.getScript("js/ptu-battle.js");
function changeView(b,a){currentView=b;0===b?($("#body-battle").removeClass("hidden"),$("#body-pokemon").addClass("hidden"),$("#body-settings").addClass("hidden"),a||renderBattler()):1===b?($("#body-battle").addClass("hidden"),$("#body-pokemon").removeClass("hidden"),$("#body-settings").addClass("hidden"),a||renderPokemonList()):2===b&&($("#body-battle").addClass("hidden"),$("#body-pokemon").addClass("hidden"),$("#body-settings").removeClass("hidden"))}
...
function changeGMView(b){currentView=b;0===b?($("#body-battle").removeClass("hidden"),$("#body-pokemon").addClass("hidden"),$("#body-settings").addClass("hidden"),renderBattler()):1===b?($("#body-battle").addClass("hidden"),$("#body-pokemon").removeClass("hidden"),$("#body-settings").addClass("hidden"),renderPokemonList()):2===b&&($("#body-battle").addClass("hidden"),$("#body-pokemon").addClass("hidden"),$("#body-settings").removeClass("hidden"))};var AfflictionEnum={Burned:"BURNED",Frozen:"FROZEN",Paralysis:"PARALYSIS",Poisoned:"POISONED",Fainted:"FAINTED",Confused:"CONFUSED"},AfflictionHelper={hasAffliction:function(b,a){return!1},addAffliction:function(b,a,c){a&&"object"!==$.type(a)&&(a=getJSONNonAsync("api/v1/data/character/"+a));b=b.toUpperCase();this.typeHasImmunity(a.Type1,b)||a.Type2&&this.typeHasImmunity(a.Type2,b)||2===DBG_MODE||$.post("api/v1/data/character/affliction",{character_id:a.CharacterId,affliction:b,method:"ADD"},function(c){b===
...
JSON.stringify({type:"afflict_add",affliction:b}));0===currentView&&renderBattler()})},deleteAffliction:function(b,a){b=b.toUpperCase();"BURNED"==b||"FROZEN"==b||"PARALYSIS"==b||"POISONED"==b?(gm_data.entities[a].tags.push(b).splice(array.indexOf(b),1),"BURNED"==b?(modifyCombatStage(a,"def",2),doToast(gm_data.entities[a].name+" was cured of their burn")):"FROZEN"==b?doToast(gm_data.entities[a].name+" was cured of Frozen"):"PARALYSIS"==b?doToast(gm_data.entities[a].name+" was cured of Paralysis"):
"POISONED"==b&&(modifyCombatStage(a,"spdef",2),doToast(gm_data.entities[a].name+" was cured of Poison"))):(gm_data.entities[a].battle_tags.push(b).splice(array.indexOf(b),1),doToast(gm_data.entities[a].name+" was cured of "+b));sendMessage(connections[a],JSON.stringify({type:"afflict_delete",affliction:b}));0==currentView&&renderBattler()},handleAffliction:function(b,a){if("Burned"==b||"Poisoned"==b){var c=gm_data.pokemon[a].level+3*gm_data.pokemon[a].hp+10;gm_data.pokemon[a].health-=Math.floor(.1*
...
function endBattle(){window.confirm("Are you sure you want to reset the battle?")&&($.each(in_battle,function(b,a){sendMessage(a,JSON.stringify({type:"battle_end"}))}),in_battle={},renderBattler())}
function renderBattler(){if(0==currentView){generateGrid($(".battle-grid"),6*parseInt($("#zoom-slider").val()));var b="";$.isEmptyObject(in_battle)?b='<h3 class="text-muted">No one\'s here yet \ud83d\ude1f</h3><h4 class="text-muted">Send your <a data-toggle="modal" data-target="#modalShare">customized link</a> to your players. Then, have them hit "Join Battle" when they\'re ready.</h4>':($.each(in_battle,function(a,c){a=gm_data.entities[c].level+3*getStatById("hp",c)+10;var d=Math.floor(getStatById("health",
c)/a*100);100<d&&console.log("Warning: Pokemon with ID "+c+" has hit points above its max: "+getStatById("health",c)+"/"+a);b+='<div class="col-md-6 col-md-offset-3 pokemon" data-name="'+c+'"><h2 class="name">'+gm_data.pokemon[c].name+'</h2><div class="progress" data-hp="'+gm_data.pokemon[c].hp+'" data-max-hp="'+gm_data.pokemon[c].max_hp+'"><div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="'+d+'" aria-valuemin="0" aria-valuemax="100" style="width: '+d+'%;"></div></div></div>'}),
b+='<br/><button class="btn btn-danger btn-raised" onclick="endBattle()">End Battle</button>');$("#tab-battle-list").html(b)}}function doGMDataConvert_v1_v2(){gm_data.entities={};$.each(gm_data.pokemon,function(b,a){a.entity_type="POKEMON";a.hp=[a.hp];a.health=[a.health];a.atk=[a.atk];a.def=[a.def];a.spatk=[a.spatk];a.spdef=[a.spdef];a.speed=[a.speed];a.notes_discovery=a.discovery;a.type=a.type.split("/");delete a.afflictions;delete a.discovery;gm_data.entities[b]=a})};var BattlerView={render:function(){if(0===currentView){generateGrid($(".battle-grid"),6*parseInt($("#zoom-slider").val()));var b=[],a="";$.isEmptyObject(battle)?a='<h3 class="text-muted">No one\'s here yet \ud83d\ude1f</h3><h4 class="text-muted">Send your <a data-toggle="modal" data-target="#modalShare">customized link</a> to your players. Then, have them hit "Join Battle" when they\'re ready.</h4>':($.each(battle,function(a,d){var c=gm_data.pokemon[a].level+3*gm_data.pokemon[a].hp+10,f=Math.floor(gm_data.pokemon[a].health/
c*100),h=gm_data.pokemon[a].injuries;100<f&&console.log("Warning: Pokemon with ID "+a+" has hit points above its max: "+gm_data.pokemon[a].health+"/"+c);var g="";null!=gm_data.pokemon[a].afflictions&&$.each(gm_data.pokemon[a].afflictions,function(a,b){g+=' <span class="label label-danger">'+b+"</span>"});null!=d.afflictions&&$.each(d.afflictions,function(a,b){g+=' <span class="label label-danger">'+a+"</span>"});d=0;for(c=gm_data.pokemon[a].speed*getStageMultiplier(battle[a].stage_speed);d<b.length&&
!(c>b[d].speed);)d++;b.splice(d,0,{html:'<div class="col-md-6 col-md-offset-3 pokemon" data-name="'+a+'"><h2 class="name">'+gm_data.pokemon[a].name+g+'</h2><div class="progress" data-hp="'+gm_data.pokemon[a].hp+'" data-max-hp="'+gm_data.pokemon[a].max_hp+'"><div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="'+f+'" aria-valuemin="0" aria-valuemax="100" style="width: '+f+'%;"></div><div class="progress-bar progress-bar-injuries pull-right" role="progressbar" aria-valuenow="'+
h+'0" aria-valuemin="0" aria-valuemax="100" style="width: '+h+'0%;"></div></div></div>',speed:c})}),$.each(b,function(b,d){a+=d.html}),a+='<br/><button class="btn btn-danger btn-raised" onclick="endBattle()">End Battle</button>');$("#tab-battle-list").html(a)}},createMoveDialog:function(b,a,c){0!==currentView&&doToast(b.Name+" used "+a+"!");b='<img src="img/pokemon/'+b.PokedexNo+'.gif" /><div class="battle-dialog"><p>'+b.Name+' used <span style="color:'+typeColor(c.Type)+'">'+a+"</span></p></div>";
$("#battle-message").html(b)},addMoveDialogInfo:function(b){$(".battle-dialog").append("<p><strong>&gt;</strong> "+b+"</p>")}},CharacterView={renderCharacterList:function(){$.getJSON("api/v1/data/character/list",{campaign_id:34},function(b){var a=$(".list-pokemon").html("");$.each(b,function(b,d){b='<img class="img-circle pull-left bg-danger" height="60px" width="60px" />';"POKEMON"===d.type&&(b='<img src="img/pokemon-profiles/'+d.dex+'.png" class="img-circle pull-left bg-danger" height="60px" width="60px" />');
a.append('<div class="char-entry char-owner col-sm-6">'+b+'<div class="btn-group-vertical pull-right"><button onclick="onClickEditCharacter(\''+d.id+'\')" class="btn btn-info btn-xs"><i class="material-icons">edit</i></button><button onclick="onClickDeletePokemon(\''+d.id+'\')" class="btn btn-danger btn-xs"><i class="material-icons">delete</i></button></div><span class="char-name">'+d.name+"</span></div>");$.each(d.owned,function(b,c){b='<img class="img-circle pull-left bg-danger" height="60px" width="60px" />';
"POKEMON"===c.type&&(b='<img src="img/pokemon-profiles/'+c.dex+'.png" class="img-circle pull-left bg-danger" height="60px" width="60px" />');a.append('<div class="char-entry col-sm-6">'+b+'<div class="btn-group-vertical pull-right"><button onclick="onClickEditCharacter(\''+c.id+'\')" class="btn btn-info btn-xs"><i class="material-icons">edit</i></button><button onclick="onClickDeletePokemon(\''+c.id+'\')" class="btn btn-danger btn-xs"><i class="material-icons">delete</i></button></div><span class="char-name">'+
c.name+"</span></div>")});a.append("<hr/>")})})},onClickEditCharacter:function(b){createCharacterEditor(b,"#tab-char-io");changeView(1,!0);$('a[href="#tab-char-io"]').tab("show")}};function saveGM(){var b="data:text/json;charset=utf-8,"+encodeURIComponent(JSON.stringify(gm_data)),a=document.getElementById("downloadAnchor");a.setAttribute("href",b);a.setAttribute("download","GMData.json");a.click();firebase.auth()}function fetchCharacterById(b,a){$.getJSON("api/v1/data/character/"+b,a)}function fetchCharactersByCampaignId(b,a){$.getJSON("api/v1/data/character/list/?campaign_id="+b,a)}
function fetchCharacterMoves(b,a){$.getJSON("/api/v1/data/character/moves/?character_id="+b,a)}function fetchPage(b,a){$.get("/pages/"+b+".php",a)}function createCharacterEditor(b,a){$.get("/pages/char-edit.php",{id:b},function(b){$(a).html(b)})}function getJSONNonAsync(b){var a;$.ajax({type:"GET",url:b,dataType:"json",async:!1,success:function(b){a=b}});return a}function getJSONNonAsync(b,a){var c;$.ajax({type:"GET",url:b,dataType:"json",async:!1,data:a,success:function(a){c=a}});return c};var connectionMap={};peer.on("connection",function(b){b.on("open",function(){receiveMessages(b,readMessage);b.send(JSON.stringify({type:"pokemon_list",pokemon:gm_data.pokemon}))})});
var GMPeer=function(){return{reconnect:function(){peer.reconnect()},readMessage:function(b,a){a=JSON.parse(a);if("alert"===a.type)doToast(a.content);else if("pokemon_get"==a.type)b.send(JSON.stringify({type:"pokemon",pokemon:gm_data.entities[a.pokemon_id]}));else if("pokemon_list"==a.type)b.send(JSON.stringify({type:"pokemon_list",pokemon:gm_data.entities}));else if("battle_add"===a.type)$.post("api/v1/battle/join",{character_id:a.character_id},function(a){"false"===a?b.send({type:"alert",content:"Already in battle"}):
alert(a)}),connections[a.character_id]=a.from,in_battle.push(a.character_id),b.send(JSON.stringify({type:"battle_added",pokemon_id:a.character_id})),doToast(gm_data.entities[a.pokemon].name+" Appears!"),renderBattler();else if("pokemon_update"==a.type)gm_data.entities[a.pokemon][a.field][0]=a.value,"health"===this.field&&(gm_data.entities[a.pokemon].health[0]=a.value);else if("pokemon_setcs"==a.type)battle[a.pokemon][a.field]=a.value;else if("pokemon_afflict_add"==a.type)AfflictionHelper.addAffliction(a.affliction,
