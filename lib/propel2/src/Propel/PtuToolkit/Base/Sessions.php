...
use \Exception;
use \PDO;
use Propel\PtuToolkit\SessionsQuery as ChildSessionsQuery;
use Propel\PtuToolkit\Users as ChildUsers;
use Propel\PtuToolkit\UsersQuery as ChildUsersQuery;
use Propel\PtuToolkit\Map\SessionsTableMap;
...
abstract class Sessions implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Propel\\PtuToolkit\\Map\\SessionsTableMap';
    ...
    /**
     * The value for the firebase_token field.
     *
     * @var        string
     */
    ...
    /**
     * Initializes internal state of Propel\PtuToolkit\Base\Sessions object.
     * @see applyDefaults()
     */
    ...
    /**
     * Get the [firebase_token] column value.
     *
     * @return string
     */
     ...
     * @return $this|\Propel\PtuToolkit\Sessions The current object (for fluent API support)
     */
    public function setSessionId($v)
    {
        ...
        if ($this->session_id !== $v) {
            $this->session_id = $v;
            $this->modifiedColumns[SessionsTableMap::COL_SESSION_ID] = true;
        }
    ...
    public function setUserFirebaseId($v)
    {
        ...
        if ($this->user_firebase_id !== $v) {
            ...
            $this->modifiedColumns[SessionsTableMap::COL_USER_FIREBASE_ID] = true;
        }
    ...
    /**
     * Set the value of [firebase_token] column.
     *
     * @param string $v new value
     * @return $this|\Propel\PtuToolkit\Sessions The current object (for fluent API support)
     */
    public function setFirebaseToken($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->firebase_token !== $v) {
            $this->firebase_token = $v;
            $this->modifiedColumns[SessionsTableMap::COL_FIREBASE_TOKEN] = true;
        }
    ...
    public function setAgent($v)
    {
        ...
        if ($this->agent !== $v) {
            $this->agent = $v;
            $this->modifiedColumns[SessionsTableMap::COL_AGENT] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : SessionsTableMap::translateFieldName('SessionId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->session_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SessionsTableMap::translateFieldName('UserFirebaseId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->user_firebase_id = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SessionsTableMap::translateFieldName('FirebaseToken', TableMap::TYPE_PHPNAME, $indexType)];
            $this->firebase_token = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SessionsTableMap::translateFieldName('IpAddress', TableMap::TYPE_PHPNAME, $indexType)];
            $this->ip_address = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SessionsTableMap::translateFieldName('Agent', TableMap::TYPE_PHPNAME, $indexType)];
            $this->agent = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : SessionsTableMap::translateFieldName('Timestamp', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Propel\\PtuToolkit\\Sessions'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null !== $this->session_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . SessionsTableMap::COL_SESSION_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(SessionsTableMap::COL_SESSION_ID)) {
            $modifiedColumns[':p' . $index++]  = 'session_id';
        }
        if ($this->isColumnModified(SessionsTableMap::COL_USER_FIREBASE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'user_firebase_id';
        }
        if ($this->isColumnModified(SessionsTableMap::COL_FIREBASE_TOKEN)) {
            $modifiedColumns[':p' . $index++]  = 'firebase_token';
        }
        if ($this->isColumnModified(SessionsTableMap::COL_IP_ADDRESS)) {
            $modifiedColumns[':p' . $index++]  = 'ip_address';
        }
        if ($this->isColumnModified(SessionsTableMap::COL_AGENT)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'user_firebase_id':
                        ...
                        break;
                    case 'firebase_token':
                        $stmt->bindValue($identifier, $this->firebase_token, PDO::PARAM_STR);
                        break;
                    case 'ip_address':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[2] => $this->getFirebaseToken(),
            $keys[3] => $this->getIpAddress(),
            $keys[4] => $this->getAgent(),
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Propel\PtuToolkit\Sessions
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setFirebaseToken($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setIpAddress($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setAgent($arr[$keys[4]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Propel\PtuToolkit\Sessions The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SessionsTableMap::COL_SESSION_ID)) {
            $criteria->add(SessionsTableMap::COL_SESSION_ID, $this->session_id);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_USER_FIREBASE_ID)) {
            $criteria->add(SessionsTableMap::COL_USER_FIREBASE_ID, $this->user_firebase_id);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_FIREBASE_TOKEN)) {
            $criteria->add(SessionsTableMap::COL_FIREBASE_TOKEN, $this->firebase_token);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_IP_ADDRESS)) {
            $criteria->add(SessionsTableMap::COL_IP_ADDRESS, $this->ip_address);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_AGENT)) {
            $criteria->add(SessionsTableMap::COL_AGENT, $this->agent);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_TIMESTAMP)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Propel\PtuToolkit\Sessions (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setFirebaseToken($this->getFirebaseToken());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Propel\PtuToolkit\Sessions Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if (null !== $this->aUsers) {
            $this->aUsers->removeSessions($this);
        }
