...
use Propel\PtuToolkit\Sessions;
use Propel\PtuToolkit\SessionsQuery;
...
class SessionsTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Propel.PtuToolkit.Map.SessionsTableMap';
     ...
     */
    const OM_CLASS = '\\Propel\\PtuToolkit\\Sessions';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Propel.PtuToolkit.Sessions';
     ...
     */
    const COL_SESSION_ID = 'sessions.session_id';

    /**
     * the column name for the user_firebase_id field
     */
    const COL_USER_FIREBASE_ID = 'sessions.user_firebase_id';

    /**
     * the column name for the firebase_token field
     */
    const COL_FIREBASE_TOKEN = 'sessions.firebase_token';

    /**
     * the column name for the ip_address field
     */
    const COL_IP_ADDRESS = 'sessions.ip_address';

    /**
     * the column name for the agent field
     */
    const COL_AGENT = 'sessions.agent';

    /**
     * the column name for the timestamp field
     */
    const COL_TIMESTAMP = 'sessions.timestamp';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('SessionId', 'UserFirebaseId', 'FirebaseToken', 'IpAddress', 'Agent', 'Timestamp', ),
        self::TYPE_CAMELNAME     => array('sessionId', 'userFirebaseId', 'firebaseToken', 'ipAddress', 'agent', 'timestamp', ),
        self::TYPE_COLNAME       => array(SessionsTableMap::COL_SESSION_ID, SessionsTableMap::COL_USER_FIREBASE_ID, SessionsTableMap::COL_FIREBASE_TOKEN, SessionsTableMap::COL_IP_ADDRESS, SessionsTableMap::COL_AGENT, SessionsTableMap::COL_TIMESTAMP, ),
        self::TYPE_FIELDNAME     => array('session_id', 'user_firebase_id', 'firebase_token', 'ip_address', 'agent', 'timestamp', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('SessionId' => 0, 'UserFirebaseId' => 1, 'FirebaseToken' => 2, 'IpAddress' => 3, 'Agent' => 4, 'Timestamp' => 5, ),
        self::TYPE_CAMELNAME     => array('sessionId' => 0, 'userFirebaseId' => 1, 'firebaseToken' => 2, 'ipAddress' => 3, 'agent' => 4, 'timestamp' => 5, ),
        self::TYPE_COLNAME       => array(SessionsTableMap::COL_SESSION_ID => 0, SessionsTableMap::COL_USER_FIREBASE_ID => 1, SessionsTableMap::COL_FIREBASE_TOKEN => 2, SessionsTableMap::COL_IP_ADDRESS => 3, SessionsTableMap::COL_AGENT => 4, SessionsTableMap::COL_TIMESTAMP => 5, ),
        self::TYPE_FIELDNAME     => array('session_id' => 0, 'user_firebase_id' => 1, 'firebase_token' => 2, 'ip_address' => 3, 'agent' => 4, 'timestamp' => 5, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, 5, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\Propel\\PtuToolkit\\Sessions');
        $this->setPackage('Propel.PtuToolkit');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('session_id', 'SessionId', 'INTEGER', true, null, null);
        $this->addForeignKey('user_firebase_id', 'UserFirebaseId', 'VARCHAR', 'users', 'firebase_id', true, 16, null);
        $this->addColumn('firebase_token', 'FirebaseToken', 'LONGVARCHAR', true, null, null);
        $this->addColumn('ip_address', 'IpAddress', 'VARCHAR', true, 16, null);
        $this->addColumn('agent', 'Agent', 'VARCHAR', true, 64, null);
  ...
  array (
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(SessionsTableMap::COL_SESSION_ID);
            $criteria->addSelectColumn(SessionsTableMap::COL_USER_FIREBASE_ID);
            $criteria->addSelectColumn(SessionsTableMap::COL_FIREBASE_TOKEN);
            $criteria->addSelectColumn(SessionsTableMap::COL_IP_ADDRESS);
            $criteria->addSelectColumn(SessionsTableMap::COL_AGENT);
            ...
            $criteria->addSelectColumn($alias . '.firebase_token');
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Propel\PtuToolkit\Sessions) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(SessionsTableMap::DATABASE_NAME);
            $criteria->add(SessionsTableMap::COL_SESSION_ID, (array) $values, Criteria::IN);
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(SessionsTableMap::COL_SESSION_ID) && $criteria->keyContainsValue(SessionsTableMap::COL_SESSION_ID) ) {
