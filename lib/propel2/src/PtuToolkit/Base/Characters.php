...
use PtuToolkit\BattleEntries as ChildBattleEntries;
use PtuToolkit\BattleEntriesQuery as ChildBattleEntriesQuery;
...
use PtuToolkit\CharacterMoves as ChildCharacterMoves;
use PtuToolkit\CharacterMovesQuery as ChildCharacterMovesQuery;
use PtuToolkit\Characters as ChildCharacters;
use PtuToolkit\CharactersQuery as ChildCharactersQuery;
use PtuToolkit\Map\BattleEntriesTableMap;
use PtuToolkit\Map\CharacterAbilitiesTableMap;
use PtuToolkit\Map\CharacterBuffsTableMap;
use PtuToolkit\Map\CharacterMovesTableMap;
...
abstract class Characters implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PtuToolkit\\Map\\CharactersTableMap';
    ...
    /**
     * Initializes internal state of PtuToolkit\Base\Characters object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\PtuToolkit\Characters The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\PtuToolkit\\Characters'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->battleEntriessScheduledForDeletion !== null) {
                if (!$this->battleEntriessScheduledForDeletion->isEmpty()) {
                    \PtuToolkit\BattleEntriesQuery::create()
            ...
            if ($this->characterAbilitiessScheduledForDeletion !== null) {
                if (!$this->characterAbilitiessScheduledForDeletion->isEmpty()) {
                    \PtuToolkit\CharacterAbilitiesQuery::create()
            ...
            if ($this->characterBuffssScheduledForDeletion !== null) {
                if (!$this->characterBuffssScheduledForDeletion->isEmpty()) {
                    \PtuToolkit\CharacterBuffsQuery::create()
            ...
            if ($this->characterMovessScheduledForDeletion !== null) {
                if (!$this->characterMovessScheduledForDeletion->isEmpty()) {
                    \PtuToolkit\CharacterMovesQuery::create()
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\PtuToolkit\Characters
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PtuToolkit\Characters The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PtuToolkit\Characters (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PtuToolkit\Characters Clone of current object.
     * @throws PropelException
     */
    ...
    public function initBattleEntriess($overrideExisting = true)
    {
        ...
        $this->collBattleEntriess->setModel('\PtuToolkit\BattleEntries');
    }
    ...
    public function initCharacterAbilitiess($overrideExisting = true)
    {
        ...
        $this->collCharacterAbilitiess->setModel('\PtuToolkit\CharacterAbilities');
    }
    ...
    public function initCharacterBuffss($overrideExisting = true)
    {
        ...
        $this->collCharacterBuffss->setModel('\PtuToolkit\CharacterBuffs');
    }
    ...
    public function initCharacterMovess($overrideExisting = true)
    {
        ...
        $this->collCharacterMovess->setModel('\PtuToolkit\CharacterMoves');
    }
